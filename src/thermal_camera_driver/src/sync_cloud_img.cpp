...
int main(int argc, char **argv) {
    ...
    std::string camera_topic, cloud_topic,camera_id;
    std::string file_saved_path;
    ph.getParam("camera_topic", camera_topic);
    ph.getParam("pointcloud_topic", cloud_topic);
    ph.getParam("camera_id", camera_id);
    ph.getParam("file_saved_path", file_saved_path);


    std::cout << "cloud_topic:" << cloud_topic << std::endl;
    std::cout << "camera_id:" << camera_id << std::endl;
    std::cout << "file_saved_path:" << file_saved_path << std::endl;

    img_save_path = file_saved_path+"/cam"+camera_id+"/";
    cloud_save_path = file_saved_path+"/cloud"+camera_id+"/";
    std::cout << "img_save_path:" << img_save_path << std::endl;
    std::cout << "cloud_save_path:" << cloud_save_path << std::endl;


    ROS_INFO("init cloud_image_sync_callback_node");
    message_filters::Subscriber<sensor_msgs::PointCloud2> cloud_sub(n, cloud_topic, 1);
    camera_topic = camera_topic + "/camera" + camera_id + "/image_raw";
    std::cout << "camera_topic:" << camera_topic << std::endl;

    message_filters::Subscriber<sensor_msgs::Image> image_sub(n, camera_topic, 1);
